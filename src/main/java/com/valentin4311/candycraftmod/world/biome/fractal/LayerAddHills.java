...
public class LayerAddHills extends Layer {
	...
	public LayerAddHills(long seed, Layer parent, Map<BiomeInfo, BiomeInfo> hillVariants,
						 Object ignored, int neighborRequirement) {
		this(seed, parent, hillVariants, ignored, neighborRequirement, null, null);
	}
	...
	protected BiomeInfo[] getNewBiomes(int x, int z, int width, int length) {
		...
		return forEachWithNeighbors(x, z, width, length, (b, ix, iz, n) -> {
			...
			if (hillVariant.equals(BiomeInfo.of(BiomeIds.DEEP_OCEAN, 1)) && nextInt(3) == 0) {
				hillVariant = BiomeInfo.of(nextInt(2) == 0 ? BiomeIds.PLAINS : BiomeIds.FOREST);
			} else if (hillVariant.is(BiomeIds.FOREST) && b.is(BiomeIds.PLAINS) && mutationData != null && nextInt(3) == 0) {
				hillVariant = hillVariants.getOrDefault(BiomeInfo.of(BiomeIds.FOREST), hillVariant);
			} else if (hillVariant.is(BiomeIds.OCEAN) && hillVariant.type() == 1) {
				hillVariant = mutationData != null ? BiomeInfo.of(BiomeIds.DEEP_OCEAN) : hillVariant.withType(0);
			}

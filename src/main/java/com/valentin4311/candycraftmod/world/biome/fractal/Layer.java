...
public abstract class Layer {
	...
	public static Layer getLayer(long seed, FractalSettings settings) {
		BiomeInfo ocean = BiomeInfo.of(BiomeIds.OCEAN);
		BiomeInfo deepOcean = BiomeInfo.of(BiomeIds.DEEP_OCEAN);
		BiomeInfo plains = settings.plains;
		BiomeInfo river = BiomeInfo.of(BiomeIds.RIVER);
		BiomeInfo frozenOcean = BiomeInfo.of(BiomeIds.FROZEN_OCEAN);
		BiomeInfo icePlains = settings.icePlains;
		BiomeInfo frozenRiver = BiomeInfo.of(BiomeIds.FROZEN_RIVER);
		BiomeInfo mushroomIsland = BiomeInfo.of(BiomeIds.MUSHROOM_FIELDS);
		BiomeInfo beach = BiomeInfo.of(BiomeIds.BEACH);
		BiomeInfo stonyShore = settings.addStonyShores ? BiomeInfo.of(BiomeIds.STONY_SHORE) : null;
		...
		Layer land;
		if (!settings.oceans) {
			land = new LayerSingleBiome(DummyBiome.PLAINS.biomeInfo);
		...
		for (int i = 0; i < settings.hillScale; i++) {
			...
			if (settings.beachShrink == i - 2 - settings.hillScale && settings.addBeaches) {
				biomes = new LayerAddEdge(1000, biomes, beach, stonyShore, ocean, mushroomIsland,
					settings.useClimaticBiomes ? null : settings.edgeVariants, null);
			}
		...
		if (settings.useClimaticBiomes) biomes = new LayerAddEdge(1000, biomes, settings.edgeVariants, null);
		if (settings.addHills) {
			...
			biomes = settings.addMutations
				? new LayerAddHills(1000, biomes, settings.hillVariants, null, neighborRequirement, mutationLayout, settings.mutatedVariants)
				: new LayerAddHills(1000, biomes, settings.hillVariants, null, neighborRequirement);
		}
		...
		if (settings.addClimaticOceans && settings.oceans) {
			...
			biomes = new LayerApplyOceanTemperature(biomes, oceanTemperature, null);
		}

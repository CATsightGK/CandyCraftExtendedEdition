...
public record AlchemyJeiRecipe(List<ItemStack> inputs, ItemStack output, ResourceLocation id) {
    public static List<AlchemyJeiRecipe> createRecipes() {
        ...
        for (ItemStack ingredient : AlchemyMixing.getDisplayIngredientStacks()) {
            ...
            if (!output.isEmpty()) {
                ...
                recipes.add(new AlchemyJeiRecipe(List.copyOf(inputs), output, new ResourceLocation(CandyCraft.MODID, "alchemy_table/" + sanitizeId(path))));
            }
        }
